...
class MaxentSolverSVD(AnalyticContinuationSolver):
    def __init__(self,im_axis,re_axis,im_data,kernel_mode='',model=None,stdev=None,beta=None,**kwargs):
        ...
        self.W2 = np.einsum('k,km,m,kn,n,ln,l,l->ml',self.E, self.U_svd, self.Xi_svd, self.U_svd, self.Xi_svd, self.V_svd, self.dw, self.model)
    ...
    def error_propagation(self,obs,args):
        hess=-self.d2chi2
        hess+=self.alpha_opt*np.diag(1./self.A_opt)*self.dw[:,None]*self.dw[None,:] # possibly, it's 2*alpha
        varmat=np.linalg.inv(hess) # TODO actually here should be a minus...
        obsval=obs(self.re_axis,args)
        integrand=obsval[:,None]*obsval[None,:]*varmat*self.dw[:,None]*self.dw[None,:]
...
class GreensFunction(object):
    def __init__(self,spectrum=None,wgrid=None,kind=''):
        ...
        self.dw=np.diff(np.concatenate(([self.wmin],(self.wgrid[1:]+self.wgrid[:-1])/2.,[self.wmax])))
